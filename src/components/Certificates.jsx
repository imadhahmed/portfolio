...
export default function Certificates() {
  ...
  const safeIdx = certificates.length > 0 ? (activeIdx >= certificates.length ? 0 : activeIdx) : 0
  ...
  const activeCert = certificates.length > 0 ? (certificates[safeIdx] || certificates[0]) : null
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        {loading ? (
          <div className="text-center py-24 border border-white/10 rounded-2xl bg-white/5 backdrop-blur-sm">
            ...
            <p className="text-sm text-gray-400 font-mono">Loading database certificates...</p>
          </div>
        ) : certificates.length === 0 ? (
          <div className="text-center py-24 border border-white/10 rounded-2xl bg-white/5 backdrop-blur-sm max-w-2xl mx-auto">
            <Award className="mx-auto h-12 w-12 text-[#00df8f]/60 mb-4" />
            <h3 className="text-xl font-bold text-white mb-2">No Database Certificates Found</h3>
            <p className="text-sm text-gray-400 max-w-md mx-auto">
              There are currently no certificates stored in the database.
            </p>
          </div>
        ) : (
          /* Main Grid */
